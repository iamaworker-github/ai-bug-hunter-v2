...
Supports: Ollama (local), Claude API, OpenAI, Grok (xAI), OpenCode Zen

Provider selection (in order of precedence):
  1. BRAIN_PROVIDER env var  (ollama | claude | openai | grok | zen)
...
API keys (env vars):
  ...
  ZEN_API_KEY         — OpenCode Zen (deepseek-v4-flash, deepseek-v4-flash-free, etc.)
...
# Wraps Ollama, Claude, OpenAI, Grok, OpenCode Zen behind a single .chat() interface.

class LLMClient:
    """
    Unified chat interface for Ollama, Groq, DeepSeek, Claude, OpenAI, Grok, and Zen.

    Usage:
        client = LLMClient()          # auto-detect provider
        client = LLMClient("groq")    # force Groq (free tier)
        client = LLMClient("claude")  # force Claude API
        client = LLMClient("zen")     # force OpenCode Zen
    ...
    Free providers:
        ...
        zen      — OpenCode Zen free models, ZEN_API_KEY (https://opencode.ai/zen)
        deepseek — very cheap,      DEEPSEEK_API_KEY (https://platform.deepseek.com)
    """

    # Priority: free-local first, free-cloud second, paid last
    PROVIDER_PRIORITY = ["zen", "ollama", "groq", "deepseek", "claude", "openai", "grok"]
    ...
    def _init_provider(self, provider: str) -> None:
        ...
        if provider == "ollama":
            ...
            self._http.headers.update({"Authorization": f"Bearer {key}",
                                       "Content-Type": "application/json"})
            self._api_base   = "https://opencode.ai/zen/v1"
            self.available   = True
            self.description = "OpenCode Zen (deepseek-v4-flash-free, etc.)"
    ...
    def chat(self, model: str | None, system: str, user: str,
        ...
        try:
            ...
            elif self.provider in ("openai", "grok", "groq", "deepseek", "zen"):
    ...
    def list_models(self) -> list[str]:
        ...
        if self.provider == "ollama" and self._ollama:
            ...
            return ["deepseek-v4-flash", "deepseek-v4-flash-free", "deepseek-v4-pro",
