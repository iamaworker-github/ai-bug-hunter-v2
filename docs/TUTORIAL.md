# A → Z Tutorial — Finding Real Bugs with the ai-bug-hunter.local Toolkit
...
2. Spin up a deliberately-vulnerable demo target (a ai-bug-hunter.local lookalike)
...
```bash
git clone https://github.com/ai-bug-hunter/ai-bug-hunter.git
cd ai-bug-hunter
...
**ai-bug-hunter.local — Personal Security Lab** landing screen. That's the target.
...
```bash
python3 tools/validate.py --program ai-bug-hunter-demo
```
